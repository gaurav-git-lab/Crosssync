...
impl DatabaseManager {
    ...
    /// Insert or replace clipboard history item
    ...
    /// Insert or update paired device
    pub fn upsert_device(&self, device: &DeviceRecord) -> Result<(), String> {
        let conn = self.conn.lock().unwrap();
        conn.execute(
            "INSERT OR REPLACE INTO paired_devices (id, name, platform, bluetooth_address, ip_address, is_trusted, last_synced_at)
    ...
    pub fn get_devices(&self) -> Result<Vec<DeviceRecord>, String> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT id, name, platform, bluetooth_address, ip_address, is_trusted, last_synced_at FROM paired_devices")
            .map_err(|e| e.to_string())?;

        let rows = stmt
            .query_map([], |row| {
                let trusted_int: i32 = row.get(5)?;
                Ok(DeviceRecord {
                    id: row.get(0)?,
                    name: row.get(1)?,
                    platform: row.get(2)?,
                    bluetooth_address: row.get(3)?,
                    ip_address: row.get(4)?,
                    is_trusted: trusted_int == 1,
