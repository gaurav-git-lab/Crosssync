...
impl ClipboardListener {
    ...
    /// Set clipboard content from external incoming message (without triggering echo ping-pong)
    pub async fn set_clipboard(&self, content: &str) -> Result<(), String> {
        ...
        println!("[Clipboard] Applied incoming synced text: {} chars", content.len());
        Ok(())
    }

    /// Check if hash exists in recent history (prevents echo loops)
    pub async fn is_duplicate(&self, hash: &str) -> bool {
        let history = self.history_hashes.lock().await;
        history.contains(&hash.to_string())
    }

    /// Run polling listener loop on Windows
        ...
        F: FnMut(String, String) + Send + 'static,
    {
        ...
        tokio::spawn(async move {
            println!("[Clipboard] Native Windows clipboard watcher started (150ms interval)");
            loop {
                {
