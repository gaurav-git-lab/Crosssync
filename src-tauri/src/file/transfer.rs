...
impl FileTransferEngine {
    ...
    /// Prepare file for sending: calculate SHA256 and chunk division
    ...
    /// Split file into chunk envelopes with CRC32 verification
    ...
    /// Write received chunk to staging directory and verify CRC
    pub async fn write_chunk(&self, chunk: &FileChunkInfo) -> Result<bool, String> {
        ...
        if computed_crc != chunk.crc32 {
            return Err(format!(
                "CRC32 mismatch on chunk {}: expected {}, computed {}",
                chunk.chunk_index, chunk.crc32, computed_crc
            ));
        }
    ...
    /// Finalize assembled file, verify SHA-256 hash, and move to user Downloads folder
    pub async fn finalize_transfer(
        ...
        if computed_hash != expected_hash {
            return Err(format!(
                "SHA-256 hash verification failed for {}. File may be corrupted.",
                file_name
            ));
        }
        ...
        println!("[File Engine] Successfully received and verified: {:?}", destination);
        Ok(destination.to_string_lossy().to_string())
    }
}

// Fallback helper for download directory
mod dirs_next {
